...
void send_buffer(char* buffer, int len) {
	...
	while (true) {
		...
		([&](){
			...
			{
				//超时，且没有ACK的数据包，重传
				if ((clock() - timer[win.front()] > TIMEOUT) && (is_ack[win.front()] == 0))
				{
					cout << "超时，重传数据包" << win.front() << endl;
					int temp = has_send - win.size();
					...
					//刷新计时器，记录重传次数
					timer[win.front()] = clock();
					fail_num[win.front()]++;

					if (fail_num[win.front()] >= 10)
					{
						exit(1);
					}
				}
		...
		if (recvfrom(m_ClientSocket, recv, 3, 0, (sockaddr*)&m_ServerAddress, &len_addr) != SOCKET_ERROR && check_sum(recv, 3) == 0 && recv[1] == ACK)
		{
			...
			if ((win.front() > SEQ_SPACE - WIN_SIZE) && ((int)recv[2] < WIN_SIZE)) {
				is_ack[(int)recv[2]] = 1;
			}
